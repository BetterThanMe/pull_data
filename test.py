...
def evaluate(dataset, num_files, params=None):
    ...
    for test_step, (x_mel, y_sed, y_doa) in enumerate(dataset):
        ...
        if (test_step + 1) * test_batch_size * N <= num_files * N:
            ...
            score_sed[test_step * test_batch_size * N: num_files * N] = ret['score_sed']
            score_doa[test_step * test_batch_size * N: num_files * N] = ret['score_doa']
...
#get dataset_train:
data_gen_train.reset_pointer()
num_files_train, inputs, label_sed, label_doa = data_gen_train.get_rest_batch()
dataset_train = tf.data.Dataset.from_tensor_slices((inputs, label_sed, label_doa)).batch(10)

data_gen_valid.reset_pointer()
num_files_valid, inputs, label_sed, label_doa = data_gen_valid.get_rest_batch()
dataset_valid = tf.data.Dataset.from_tensor_slices((inputs, label_sed, label_doa)).batch(10)

data_gen_test.reset_pointer()
num_files_test, inputs, label_sed, label_doa = data_gen_test.get_rest_batch()
dataset_test = tf.data.Dataset.from_tensor_slices((inputs, label_sed, label_doa)).batch(10)

#train_loop(params, scheduler, dataset_train)
print(evaluate(dataset_test, num_files_test, params))