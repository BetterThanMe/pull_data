...
def eval_step(params, x_mel, y_sed, y_doa):
    ...
    pred_sed, pred_doa = model(x_mel)
    l2_loss = tf.add_n([tf.nn.l2_loss(v) for v in model.trainable_variables])
    loss = Loss(pred_sed=pred_sed, gt_sed=y_sed, pred_doa=pred_doa, gt_doa=y_doa, l2_loss=l2_loss,
                l2_reg_lambda=params['l2_reg_lambda'])
    loss_sed, loss_doa, loss_total, loss_total_l2 = loss()
...
def train_step(params, step, x_mel, y_sed, y_doa, LR):
    ...
    with tf.GradientTape(persistent=True) as tape:
        ...
        loss = Loss(pred_sed=pred_sed, gt_sed=y_sed, pred_doa=pred_doa, gt_doa=y_doa, l2_loss=l2_loss,
                    l2_reg_lambda=params['l2_reg_lambda'])
        train_loss_sed, train_loss_doa, train_loss_total, train_loss_total_l2 = loss()

    grads = tape.gradient(train_loss_total_l2, model.trainable_variables)
    optimizer.apply_gradients(zip(grads, model.trainable_variables))

    time_str = datetime.now().isoformat()
    print("{}: step {}, loss SED {} DOA {} OUTPUT {} TOTAL {}".format(time_str, step, train_loss_sed, train_loss_doa,
    ...
    for epoch in range(scheduler['training_epoch']):
        ...
        while step<= train_batches_per_epoch:
            ...
            if current_step % evaluate_every:
                ...
                valid_score_sed = evaluation_metrics.reshape_3Dto2D(valid_score_sed) > 0.5
                valid_doa_pred = evaluation_metrics.reshape_3Dto2D(valid_score_doa)
                test_score_sed = evaluation_metrics.reshape_3Dto2D(test_score_sed) > 0.5
                test_doa_pred = evaluation_metrics.reshape_3Dto2D(test_score_doa)
...
'''
print(x_mel.shape, ' - ', y_sed.shape, ' - ', y_doa.shape)
'''
