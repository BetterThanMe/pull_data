...
class FeatureClass:
    def __init__(self, params, is_eval=False):
        ...
        # Input directories
        self._feat_label_dir = params['feat_label_dir'] # feature directory
        self._dataset_dir = params['dataset_dir']
        # data set
        self._dataset_combination = '{}_{}'.format(params['dataset'], 'eval' if is_eval else 'dev') #param['dataset']={foa, mic}
        # path to audio files
        self._aud_dir = os.path.join(self._dataset_dir, self._dataset_combination)
        # path to meta files
        self._desc_dir = None if is_eval else os.path.join(self._dataset_dir, 'metadata_dev')  #groundtruth
        ...
        self._fs = params['fs']
        self._hop_len_s = params['hop_len_s'] # hop len in second 0.02
        self._hop_len = int(self._fs * self._hop_len_s) # hop len in samples

        self._label_hop_len_s = params['label_hop_len_s'] # hop len in seconds 0.1
        self._label_hop_len = int(self._fs * self._label_hop_len_s) # hop len in samples
        self._label_frame_res = self._fs / float(self._label_hop_len) # label frame resolution
        self._nb_label_frames_1s = int(self._label_frame_res) # number of labels in 1 second

        self._win_len = 2 * self._hop_len # window length (twice of hop len)
        self._nfft = self._next_greater_power_of_2(self._win_len) # nfft
        self._nb_mel_bins = params['nb_mel_bins'] # n mel bins
        # mel points in linear frequency scale
    ...
    def _spectrogram(self, audio_input):
        _nb_ch = audio_input.shape[1] # number of channels
    ...
    # log-mel spectrogram from linear spectrogram
    def _get_mel_spectrogram(self, linear_spectra):
        mel_feat = np.zeros((linear_spectra.shape[0], self._nb_mel_bins, linear_spectra.shape[-1]))
        for ch_cnt in range(linear_spectra.shape[-1]):
            mag_spectra = np.abs(linear_spectra[:, :, ch_cnt])**2
            mel_spectra = np.dot(mag_spectra, self._mel_wts) # same if I want to extract gammatone
            log_mel_spectra = librosa.power_to_db(mel_spectra)
            mel_feat[:, :, ch_cnt] = log_mel_spectra
        mel_feat = mel_feat.reshape((linear_spectra.shape[0], self._nb_mel_bins * linear_spectra.shape[-1]))
        return mel_feat

    # get foa features from linear spectrogram
    ...
    def write_output_format_file_submission(self, _output_format_file, _output_format_dict):
        """
        Writes DCASE output format csv file, given output format dictionary
        ...
        for _frame_ind in _output_format_dict.keys():
            for _value in _output_format_dict[_frame_ind]:
                # Write Cartesian format output. Since baseline does not estimate track count we use a fixed value.
                _fid.write('{},{},{},{}\n'.format(int(_frame_ind), int(_value[0]), int(round(_value[1])), int(round(_value[2]))))
        _fid.close()
