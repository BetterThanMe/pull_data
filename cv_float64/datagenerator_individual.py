...
class DataGenerator_Individual:
    def __init__(self, label_dir, feat_dir, filename, Ncat=14, seq_len=300, seq_hop=300, iseval=False):
        self._feat2label_factor = 5 #num_feat_frames/num_label_frames = 5
    ...
    def load_data(self):
        ...
        X = np.load(os.path.join(self._feat_dir, self._filename)) #X shape = (3000, 448)
        self._X_mel = np.reshape(X, [X.shape[0], self._ndim, X.shape[-1]//self._ndim]) #(3000, 64, 7)
        if self._iseval is False:
            ...
            # dummy label - fake label
            # 4: for 4 label: SED, x_doa, y_doa, z_doa
            #(600, 56)
        ...
        self._data_index = np.arange(0, len(self._X_mel) - self._seq_len + 1, self._seq_hop) #(0, 2701, 300)
        self._data_size = len(self._data_index)
        self._feat_shape = self.get_feat_shape() #(300, 64, 7)
        self._label_shape_sed = self.get_label_shape_sed() #(60, 14)
        self._label_shape_doa = self.get_label_shape_doa() #(60, 42)
    ...
    def get_random_sample(self,N=1):
        ...
        x_mel.astype(np.float64)
        y_sed.astype(np.float64)
        y_doa.astype(np.float64)
