...
class DataRegulator:
    ...
    def get_file_list(self):
        for filename in os.listdir(self._feat_dir): #eg of filename: fold1_room1_mix001_ov1.npy -> filename[4] = 1
    ...
    def prepare_all_label_2d(self):
        ...
        self._all_label_sed_2d = np.ndarray([self._data_size * N, self._Ncat]).astype(np.float64)
        self._all_label_doa_2d = np.ndarray([self._data_size * N, self._Ncat*3]).astype(np.float64)
